...
import { RiCheckboxBlankCircleLine, RiCheckboxCircleLine } from 'oh-vue-icons/icons'
...
const emit = defineEmits<{
  (e: '@checkTask', payload: number): void
  (e: '@unCheckTask', payload: number): void
...
function unCheckTask(id: number): void {
  emit('@unCheckTask', id)
  props.tasks?.find((e) => (e.id === id ? (e.status = 'active') : ''))
}
...
<template>
  <article class="task-container" v-if="props.tasks">
    <div v-for="task in props.tasks" :key="task.id">
      ...
      <span class="cancel" @click="deleteTask(task.id)"><span>x</span></span>
    </div>
  </article>
</template>
